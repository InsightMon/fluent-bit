...
struct flb_logstore_dynamic_tenant_id_entry {
    flb_sds_t value;
...
static struct flb_logstore_dynamic_tenant_id_entry *dynamic_tenant_id_create() {
    struct flb_logstore_dynamic_tenant_id_entry *entry;

    entry = (struct flb_logstore_dynamic_tenant_id_entry *) \
        flb_calloc(1, sizeof(struct flb_logstore_dynamic_tenant_id_entry));
...
static void dynamic_tenant_id_destroy(struct flb_logstore_dynamic_tenant_id_entry *entry) {
    if (entry != NULL) {
        if (entry->value != NULL) {
...
static void flb_logstore_kv_init(struct mk_list *list)
{
    mk_list_init(list);
}
...
static flb_sds_t normalize_ra_key_name(struct flb_logstore *ctx,
...
void flb_logstore_kv_destroy(struct flb_logstore_kv *kv)
{
    /* destroy key and value */
    flb_sds_destroy(kv->key);
    if (kv->val_type == FLB_LOGSTORE_KV_STR) {
        flb_sds_destroy(kv->str_val);
    }
    else if (kv->val_type == FLB_LOGSTORE_KV_RA) {
...
int flb_logstore_kv_append(struct flb_logstore *ctx, char *key, char *val)
{
    int ra_count = 0;
    int k_len;
    int ret;
    struct flb_logstore_kv *kv;
    ...
    kv = flb_calloc(1, sizeof(struct flb_logstore_kv));
    if (!kv) {
        flb_errno();
        return -1;
    }
    ...
    if (key[0] == '$' && val == NULL) {
        ...
        kv->val_type = FLB_LOGSTORE_KV_RA;
        ...
        kv->val_type = FLB_LOGSTORE_KV_STR;
...
static void flb_logstore_kv_exit(struct flb_logstore *ctx)
{
    struct mk_list *tmp;
    struct mk_list *head;
    struct flb_logstore_kv *kv;
    ...
    mk_list_foreach_safe(head, tmp, &ctx->labels_list) {
        kv = mk_list_entry(head, struct flb_logstore_kv, _head);
...
static flb_sds_t pack_labels(struct flb_logstore *ctx,
    ...
    mk_list_foreach(head, &ctx->labels_list) {
        ...
        if (kv->ra_key != NULL && kv->ra_val == NULL) {
            ...
            if (!ra_val || flb_sds_len(ra_val) == 0) {
                ...
                /* We skip the first '$' character since it won't be valid in Logstore */
...
static int create_label_map_entry(struct flb_logstore *ctx,
                                  struct flb_sds_list *list, msgpack_object *val, int *ra_used)
{
    ...
    case MSGPACK_OBJECT_STR:
        ...
        ret = flb_logstore_kv_append(ctx, label_key, val_str);
        flb_sds_destroy(label_key);
...
static int create_label_map_entries(struct flb_logstore *ctx,
...
static int load_label_map_path(struct flb_logstore *ctx, flb_sds_t path, int *ra_used)
{
    int ret;
...
static int parse_labels(struct flb_logstore *ctx)
{
    int ret;
    int ra_used = 0;
    char *p;
    ...
    flb_logstore_kv_init(&ctx->labels_list);
    ...
    if (ctx->labels) {
        mk_list_foreach(head, ctx->labels) {
            ...
            if (entry->str[0] == '$') {
                ret = flb_logstore_kv_append(ctx, entry->str, NULL);
                if (ret == -1) {
                    return -1;
                }
                else if (ret > 0) {
            ...
            ret = flb_logstore_kv_append(ctx, key, val);
            flb_sds_destroy(key);
            flb_sds_destroy(val);
...
static int prepare_remove_keys(struct flb_logstore *ctx)
{
    int ret;
    int len;
    int size;
...
static void logstore_config_destroy(struct flb_logstore *ctx)
{
    if (ctx->u) {
    ...
    flb_logstore_kv_exit(ctx);
    flb_free(ctx);
}

static struct flb_logstore *logstore_config_create(struct flb_output_instance *ins,
                                           struct flb_config *config)
{
    int ret;
    int io_flags = 0;
    struct flb_logstore *ctx;
    struct flb_upstream *upstream;
    char *compress;

    /* Create context */
    ctx = flb_calloc(1, sizeof(struct flb_logstore));
    if (!ctx) {
        flb_errno();
        return NULL;
    }
    ...
    flb_output_net_default(FLB_LOGSTORE_HOST, FLB_LOGSTORE_PORT, ins);
    ...
    if (strcasecmp(ctx->line_format, "json") == 0) {
        ctx->out_line_format = FLB_LOGSTORE_FMT_JSON;
    }
    else if (strcasecmp(ctx->line_format, "key_value") == 0) {
        ctx->out_line_format = FLB_LOGSTORE_FMT_KV;
    }
    else {
...
static int get_tenant_id_from_record(struct flb_logstore *ctx, msgpack_object *map,
...
static int pack_record(struct flb_logstore *ctx,
                       ...
                       flb_sds_t *dynamic_tenant_id)
{
    ...
    if (ctx->drop_single_key == FLB_TRUE && rec->type == MSGPACK_OBJECT_MAP && rec->via.map.size == 1) {
        if (ctx->out_line_format == FLB_LOGSTORE_FMT_JSON) {
            rec = &rec->via.map.ptr[0].val;
        } else if (ctx->out_line_format == FLB_LOGSTORE_FMT_KV) {
...
static int cb_logstore_init(struct flb_output_instance *ins,
                        struct flb_config *config, void *data)
{
    ...
    ctx = logstore_config_create(ins, config);
    if (!ctx) {
...
static flb_sds_t logstore_compose_payload(struct flb_logstore *ctx,
                                      int total_records,
                                      char *tag, int tag_len,
                                      ...
                                      flb_sds_t *dynamic_tenant_id)
{
    ...
    /*
     * Fluent Bit uses Logstore API v1 to push records in JSON format, this
...
static void cb_logstore_flush(struct flb_event_chunk *event_chunk,
                          ...
                          struct flb_config *config)
{
    ...
    struct flb_logstore *ctx = out_context;
    struct flb_connection *u_conn;
    struct flb_http_client *c;
    struct flb_logstore_dynamic_tenant_id_entry *dynamic_tenant_id;
    struct mk_list *head;
    ...
    payload = logstore_compose_payload(ctx,
    ...
    flb_http_add_header(c,
                        FLB_LOGSTORE_CT, sizeof(FLB_LOGSTORE_CT) - 1,
                        FLB_LOGSTORE_CT_JSON, sizeof(FLB_LOGSTORE_CT_JSON) - 1);
    ...
    if (dynamic_tenant_id->value != NULL) {
        flb_http_add_header(c,
                            FLB_LOGSTORE_HEADER_SCOPE, sizeof(FLB_LOGSTORE_HEADER_SCOPE) - 1,
                            dynamic_tenant_id->value,
    ...
    if (ret == 0) {
        ...
        if (c->resp.status == 400) {
            /*
             * Logstore will return 400 if incoming data is out of order.
...
static int cb_logstore_exit(void *data, struct flb_config *config)
{
    struct flb_logstore *ctx = data;
...
static struct flb_config_map config_map[] = {
    {
     FLB_CONFIG_MAP_STR, "uri", FLB_LOGSTORE_URI,
     0, FLB_TRUE, offsetof(struct flb_logstore, uri),
     "Specify a custom HTTP URI. It must start with forward slash."
    },

    {
     FLB_CONFIG_MAP_STR, "tenant_id", NULL,
     0, FLB_TRUE, offsetof(struct flb_logstore, tenant_id),
     "Tenant ID used by default to push logs to Logstore. If omitted or empty "
     "it assumes Logstore is running in single-tenant mode and no X-Scope-OrgID "
     "header is sent."
    },

    {
     FLB_CONFIG_MAP_STR, "tenant_id_key", NULL,
     0, FLB_TRUE, offsetof(struct flb_logstore, tenant_id_key_config),
...

    {
     ...
     0, FLB_TRUE, offsetof(struct flb_logstore, labels),
...

    {
     ...
     0, FLB_TRUE, offsetof(struct flb_logstore, auto_kubernetes_labels),
     "If set to true, it will add all Kubernetes labels to Logstore labels.",
    },

    {
     FLB_CONFIG_MAP_BOOL, "drop_single_key", "false",
     0, FLB_TRUE, offsetof(struct flb_logstore, drop_single_key),
     "If set to true and only a single key remains, the log line sent to Logstore "
     "will be the value of that key.",
    },

    {
     FLB_CONFIG_MAP_CLIST, "label_keys", NULL,
     0, FLB_TRUE, offsetof(struct flb_logstore, label_keys),
     "Comma separated list of keys to use as stream labels."
    },

    {
     FLB_CONFIG_MAP_CLIST, "remove_keys", NULL,
     0, FLB_TRUE, offsetof(struct flb_logstore, remove_keys),
     "Comma separated list of keys to remove."
    },

    {
     FLB_CONFIG_MAP_STR, "line_format", "json",
     0, FLB_TRUE, offsetof(struct flb_logstore, line_format),
     "Format to use when flattening the record to a log line. Valid values are "
     "'json' or 'key_value'. If set to 'json' the log line sent to Logstore will be "
...

    {
     ...
     0, FLB_TRUE, offsetof(struct flb_logstore, label_map_path),
     "A label map file path"
    },

    {
     FLB_CONFIG_MAP_STR, "http_user", NULL,
     0, FLB_TRUE, offsetof(struct flb_logstore, http_user),
     "Set HTTP auth user"
    },

    {
     FLB_CONFIG_MAP_STR, "http_passwd", "",
     0, FLB_TRUE, offsetof(struct flb_logstore, http_passwd),
     "Set HTTP auth password"
    },

    {
     FLB_CONFIG_MAP_STR, "bearer_token", NULL,
     0, FLB_TRUE, offsetof(struct flb_logstore, bearer_token),
     "Set bearer token auth"
    },

    {
     FLB_CONFIG_MAP_SLIST_1, "header", NULL,
     FLB_CONFIG_MAP_MULT, FLB_TRUE, offsetof(struct flb_logstore, headers),
...
/* for testing */
static int cb_logstore_format_test(struct flb_config *config,
                               ...
                               void **out_data, size_t *out_size)
{
    ...
    flb_sds_t payload = NULL;
    flb_sds_t dynamic_tenant_id;
    struct flb_logstore *ctx = plugin_context;
    ...
    payload = logstore_compose_payload(ctx, total_records,
...
/* Plugin reference */
struct flb_output_plugin out_logstore_plugin = {
    .name        = "logstore",
    .description = "Logstore",
    .cb_init     = cb_logstore_init,
    .cb_flush    = cb_logstore_flush,
    .cb_exit     = cb_logstore_exit,
    .config_map  = config_map,

    /* for testing */
    .test_formatter.callback = cb_logstore_format_test,
